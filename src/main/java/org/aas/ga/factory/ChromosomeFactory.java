...
public class ChromosomeFactory<C extends Chromosome> extends AbstractGeneticFactory<C> {
    ...
    public ChromosomeFactory(Class<? extends C> clazz, AlleleOptions options, GeneFactory geneFactory)
    {
        super((Class<C>) clazz,DEFAULT_STORAGE_TYPE,DEFAULT_STORAGE_TYPE,options,new Random(),DEFAULT_SIZE);
        this.geneFactory = geneFactory;
    }
    public ChromosomeFactory(Class<? extends C>clazz, AlleleOptions options, GeneFactory geneFactory, Random seed){
        super((Class<C>) clazz,DEFAULT_STORAGE_TYPE,DEFAULT_STORAGE_TYPE,options,seed,DEFAULT_SIZE);
        this.geneFactory = geneFactory;
    }
    public ChromosomeFactory(Class<? extends C>clazz, AlleleOptions options, GeneFactory geneFactory, Random seed, int length) {
        super((Class<C>) clazz, DEFAULT_STORAGE_TYPE,DEFAULT_STORAGE_TYPE, options, seed, length);
        this.geneFactory = geneFactory;
    }
    public ChromosomeFactory(Class<? extends C> clazz, Class<? extends Collection>dataStructure, Class<? extends Collection>ds2, AlleleOptions options, GeneFactory geneFactory, Random seed, int length){
